use crate::download::downloader::{Downloader, DownloadProgress};
...
pub enum PlaylistResponse {
    Imported { id: i64, videos: Vec<crate::api::invidious::PlaylistVideo> },
    Refreshed { id: i64, videos: Vec<crate::api::invidious::PlaylistVideo> },
    Error(String),
}
...
impl App {
        ...
        while let Ok(response) = self.playlist_add_rx.try_recv() {
            match response {
                PlaylistAddResponse::Success(video_id) => {
                    self.set_error(format!("Video {} added to playlist successfully!", video_id));
                }
    ...
    fn render_history(&mut self, f: &mut ratatui::Frame) {
        let items: Vec<ListItem> = self
            ...
            .map(|entry| {
                ...
                ListItem::new(format!(
                    "{} - {} [{}]",
                    entry.title, channel, entry.watched_at
                ))
            })
            .collect();
        ...
        let list = List::new(items)
            .block(
                Block::default()
                    .title(" Watch History (Enter: Play, c: Clear, Esc: Back) ")
                    .borders(Borders::ALL)
                    .border_style(self.theme.border),
            )
            .highlight_style(
        ...
        f.render_stateful_widget(list, f.area(), &mut self.history_state);
    }
    ...
    fn render_saved(&mut self, f: &mut ratatui::Frame) {
        let items: Vec<ListItem> = self
            ...
            .map(|video| {
                ...
                ListItem::new(format!(
                    "{} - {} [{}]",
                    video.title, channel, video.saved_at
                ))
            })
            .collect();
        ...
        let list = List::new(items)
            .block(
                Block::default()
                    .title(" Saved Videos (Enter: Play, s: Unsave, Esc: Back) ")
                    .borders(Borders::ALL)
                    .border_style(self.theme.border),
            )
            .highlight_style(
        ...
        f.render_stateful_widget(list, f.area(), &mut self.saved_state);
    }
    ...
    fn render_playlist(&mut self, f: &mut ratatui::Frame) {
        if let Some(prompt_mode) = self.playlist_prompt_mode {
            let title = match prompt_mode {
                ...
                PlaylistPromptMode::Import => " Import YouTube Playlist URL (Enter to import, Esc to cancel) ",
            };
        ...
        if !self.playlist_videos.is_empty() {
            ...
            let list = List::new(items)
                .block(
                    Block::default()
                                .title(format!(" Playlist: {} (Enter: Play, d: Download, x: Remove, Esc: Back) ", playlist_name))
    ...
    fn render_search(&mut self, f: &mut ratatui::Frame) {
        ...
        if self.is_searching {
            components::render_loading(f, chunks[1], "Searching Invidious...");
            ...
            let results: Vec<ListItem> = self
                ...
                .iter()
                .map(|v| {
                    let duration = v.length_seconds.map(|s| format!("{}:{:02}", s / 60, s % 60)).unwrap_or_default();
                    ListItem::new(format!(
                        "{} - {} [{}]",
                        v.title,
                        v.author.as_deref().unwrap_or("Unknown"),
                        duration
                    ))
                })
                .collect();
                          ...
                          if let Some(prompt_mode) = self.playlist_prompt_mode {
                              match key.code {
                                  ...
                                  KeyCode::Enter => {
                                      ...
                                      self.playlist_prompt.clear();
                                      match prompt_mode {
                                          PlaylistPromptMode::New => self.handle_create_playlist(prompt),
                                          PlaylistPromptMode::Import => self.handle_import_playlist(prompt),
                                      }
                                  }
                                  KeyCode::Esc => {
                                      self.playlist_prompt_mode = None;
                              ...
                              match key.code {
                                  ...
                                  KeyCode::Down => {
                                   ...
                                   KeyCode::Enter => {
                                       if let Some((video_id, title, channel)) = self.pending_playlist_add.take() {
                                           if let Some(idx) = self.playlist_state.selected() {
                                               if let Some(playlist) = self.playlist_results.get(idx) {
                                                   ...
                                                   tokio::spawn(async move {
                                                       match db.add_to_playlist(playlist_id, &video_id, &title, channel.as_deref()) {
                                                           Ok(_) => {
                                                               let _ = tx.send(PlaylistAddResponse::Success(video_id)).await;
                                                           }
                                                           Err(e) => {
                                                               let _ = tx.send(PlaylistAddResponse::Error(e.to_string())).await;
                                                           }
                                                       }
                                                   });
                                               }
                                  ...
                                  KeyCode::Char('i') => {
                                   ...
                                   KeyCode::Char('r') => {
                                       if let Some(idx) = self.playlist_state.selected() {
                                           if let Some(playlist) = self.playlist_results.get(idx).cloned() {
                                    ...
                                    KeyCode::Char('x') => {
                                        if !self.playlist_videos.is_empty() {
                                            if let Some(idx) = self.playlist_videos_state.selected() {
                                                if let Some(video) = self.playlist_videos.get(idx).cloned() {
    ...
    fn handle_import_playlist(&mut self, prompt: String) {
        ...
        let playlist_id = if prompt.contains("list=") {
            prompt.split("list=").nth(1).and_then(|s| s.split('&').next()).map(|s| s.to_string())
        } else {
            Some(prompt.clone())
        };
            ...
            loop {
                ...
                match client.get_playlist(&playlist_id).await {
                    Ok(details) => {
                        match db.create_imported_playlist(&details.title, &playlist_id) {
                            Ok(id) => {
                                ...
                                let _ = tx.send(PlaylistResponse::Imported {
                                    id,
                                    videos: details.videos,
                                }).await;
                            }
                            Err(e) => {
                                let _ = tx.send(PlaylistResponse::Error(format!("DB Error: {}", e))).await;
                            }
                        }
                    ...
                    Err(crate::api::invidious::InvidiousError::BadInstance) if attempts < 3 => {
                        current_url = crate::api::health::rotate_to_healthy_invidious().await.to_string();
                        attempts += 1;
                    }
                    Err(e) => {
                    ...
                    Ok(details) => {
                        match db.clear_imported_playlist(playlist_id) {
                            Ok(_) => {
                                ...
                                let _ = tx.send(PlaylistResponse::Refreshed {
                                    id: playlist_id,
                                    videos: details.videos,
                                }).await;
                            }
                ...
                if let Ok(videos) = self.db.get_playlist_videos(video.playlist_id) {
                    ...
                    if let Some(selected) = self.playlist_videos_state.selected() {
                        if selected >= self.playlist_videos.len() {
                            self.playlist_videos_state.select(Some(self.playlist_videos.len().saturating_sub(1)));
                        }
                    }
                }
            }
    ...
    fn get_selected_video_id(&self) -> Option<String> {
        match self.mode {
            AppMode::Main => None,
            AppMode::Search => self.list_state.selected()
                .and_then(|idx| self.search_results.get(idx))
                .map(|v| v.video_id.clone()),
            AppMode::History => self.history_state.selected()
                .and_then(|idx| self.history_results.get(idx))
                .map(|v| v.video_id.clone()),
            AppMode::Saved => self.saved_state.selected()
                .and_then(|idx| self.saved_results.get(idx))
                .map(|v| v.video_id.clone()),
            AppMode::Playlist => {
                if !self.playlist_videos.is_empty() {
                    self.playlist_videos_state.selected()
    ...
    fn trigger_download(&mut self, video_id: String) {
        ...
        tokio::spawn(async move {
            let mut rx = Downloader::download(
                &video_id,
                &download_path,
                None,
            );
